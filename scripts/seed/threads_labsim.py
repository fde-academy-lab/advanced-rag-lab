...
THREADS = [
{
 ...
 "title": "Drills — the fifteen-minute pathway, and the three where you predict before you look",
 "body": 'Nine drills. Each is one idea and the single check that carries it, and each takes\nunder fifteen minutes. They sit in the same prerequisite graph as the units, so clearing one\nunlocks what comes after it — and four of them have no prerequisites at all, which means a\nfirst evening can be four short clears instead of one long one.\n\n| id | drill | mode | difficulty | min | needs |\n|---|---|---|---|---|---|\n| `CD1` | One line makes every request a cache miss. Move it, do not delete it. | `diagnose` | easy | ~10 | — |\n| `ED1` | This full-chain recall returns 0.667 on a question it did not answer. Fix it. | `diagnose` | easy | ~10 | — |\n| `FD1` | Three chunkers, one of them loses the end of every document — which? | `answer` | easy | ~8 | — |\n| `RD1` | Reciprocal rank fusion, with the two blanks everyone fills in wrong | `implement` | easy | ~10 | — |\n| `ED2` | Four deltas, four intervals. Which of them would you ship on? | `answer` | medium | ~8 | ED1 |\n| `RD2` | Write down what fusion will score before you look. Then look. | `answer` | medium | ~10 | RD1 |\n| `XD1` | k goes from 5 to 10. Which way does context precision move, and by how much? | `answer` | medium | ~8 | ED1 |\n| `AD1` | An agent that keeps searching after it has the answer is paying for confidence it already had | `implement` | hard | ~15 | ED1, RD1 |\n| `ED3` | Five claims in a write-up. Three of them are the reason this repo has an ADR-0015. | `answer` | hard | ~12 | ED2 |\n\n**Start anywhere in:** `CD1`, `ED1`, `FD1`, `RD1`.\n\n### The `answer` drills are the ones nothing else has\n\n`FD1`, `ED2`, `RD2`, `XD1`, `ED3` ask you to **write down a number before you open the measurement note**, post\nit, and let the grader tell you how far off you were. Being wrong is the product: the distance\nbetween your prediction and the measured value names the model of the system you were carrying,\nwhich is the one thing a passing test never does. This repository carried the wrong one for\nmonths, in print — `RD2` is that finding turned into eight minutes of practice.\n\n### How a drill thread works\n\nSame as a unit. New thread in this category, pick the drill from the dropdown, fill the form —\nfor an `answer` drill the field is **Your answer.yaml** — and the bot replies with the named\nchecks that failed, **what to work on** for each (one line, written by whoever wrote the check),\nwhat to read next, and what is unlocked. It also labels the thread: `drill`, `difficulty: …`,\n`area: …`, and `cleared` when it passes.\n\nSo `label:drill label:"difficulty: easy" -label:cleared` is your to-do list.\n\nThen, as comments:\n\n| | |\n|---|---|\n| `/why <check name>` | what one failing check is guarding against |\n| `/hint` | the next hint from the brief |\n| `/progress` | what *you* have cleared and attempted, and what is unlocked — nobody else\'s row |\n| `/solution` | the worked answer, once the thread clears |\n\n### What a drill is not\n\nIt is not a quiz. A quiz has a key somebody typed; `FD1` **runs the three chunkers** and computes\nwhich ones drop the tail, so if somebody edits the brief\'s code the key moves with it — and that\ncaught a wrong key in the drill\'s own first draft. Where a key has to be pinned (a measured\nnumber), a test holds it to the note that regenerates it, so the drill fails CI before it\nteaches a stale figure.\n\nLocally: `python -m labsim list` shows drills beside units; `python -m labsim next` picks.\n',
 "replies": [
  {"by": "maintainer", "body": "One more thing worth saying about the difficulty tags, because people read them as a\nranking of learners and they are a ranking of *drills*.\n\n`easy` means one idea with no setup. `medium` means the idea meets a real number from this\ncorpus. `hard` means two ideas that interact — `ED3` is hard because spotting a smell needs both\nthe interval rule from `ED2` and the denominator rule from `XD1`, and `AD1` is hard because\nsufficiency is `ED1`'s all-or-nothing rule wearing a loop.\n\nNothing about the tag says how long *you* will take. The minutes column is the median for\nsomebody who has read the brief once.",
   "accepted": True},
 ],
},
{
 ...
 "body": f"""Seven units and nine drills. Six modes. Every unit graded against a live retrieval corpus
rather than against a fixture; every drill one idea in under fifteen minutes.
